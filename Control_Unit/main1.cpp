...
int sc_main (int argc, char *argv[]) {
    ...
    //scControlUnit CU ("Control_Unit_Symbol");
    scInstrNumber InstrNb ("Instruction_Number");
    scInstruction InstrSel("Instruction_Selection");
    mux2to1EOI muxEOI ("Mux_EOI");
    mux2to1EOF muxEOF ("Mux_EOF");
    ...
    AddCounter.uCYCLE(uCYCLE);
    AddCounter.INUM_i(INUM);
    ...
    ROMmem.clock(clock);
     ...
     sc_trace(wf,EOI_0x00,"EOI_0x00");

     sc_trace(wf,uADDR,"uADDR");
     sc_trace(wf,uINSTR,"uINSTR");

    instruction.write(0x8000000C);
    waitMEM.write(0);
    RST.write(0);
